...
pub fn all_routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/events/public/:token",      get(public_events::get_public))
        ...
        .route("/polls/text-answers",        get(polls::get_text_answers))
        .route("/polls/templates",           get(polls::list_templates).post(polls::create_template).delete(polls::delete_template))
}